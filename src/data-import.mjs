...
const RANK_FIELD_ALIASES = {
  artCategory: ["艺考类别", "类别", "专业类别", "艺术类别", "科类方向名称", "科类方向"],
  score: ["专业分", "专业成绩", "统考分", "分数", "成绩"],
  rank: ["位次号", "累计位次", "专业位次", "位次", "排名", "累计人数", "本段累计"]
};
...
function detectField(headers, aliases) {
...
export function estimateRankFromScore(records, artCategory, score) {
  const currentScore = toNumber(score);
  if (!currentScore) return null;

  const category = normalizeArtCategory(artCategory);
  const scopedRows = (Array.isArray(records) ? records : [])
    .filter((row) => {
