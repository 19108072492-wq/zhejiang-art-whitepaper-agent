...
const simpleNarrativeFields = ["headline", "stageGoalInsight", "scoreInsight", "gapReason", "schoolTierInsight", "nextStep"];
const simpleNarrativeLimits: Record<string, number> = {
  headline: 30,
  stageGoalInsight: 64,
  scoreInsight: 58,
  gapReason: 58,
...
function buildSimpleNarrativeFallback(simplePayload: JsonObject) {
  ...
  const context = asObject(simplePayload.context);
  const contextAnalysis = asObject(simplePayload.contextAnalysis);
  const studentStage = String(context.studentStage || "高三下学期");
  const planningGoal = String(context.planningGoal || "先保本科");
  const familyBoundary = String(context.familyBoundary || "暂不确定");
  const cultureTotal = Number(simplePayload.cultureTotal || 0);
  const professionalScore = Number(simplePayload.professionalScore || 0);
  ...
  return {
    headline: cleanSimpleText(`当前综合分 ${current}，先看层次`, simpleNarrativeLimits.headline),
    stageGoalInsight: cleanSimpleText(contextAnalysis.stageGoalInsight || `${studentStage}，${planningGoal}；${familyBoundary}。先按当前分数校准路径。`, simpleNarrativeLimits.stageGoalInsight),
    scoreInsight: cleanSimpleText(rank ? `估算位次约 ${rank} 名，距目标还差 ${gap} 分。` : `当前距目标还差 ${gap} 分，位次待校准。`, simpleNarrativeLimits.scoreInsight),
    gapReason: cleanSimpleText(gapReason, simpleNarrativeLimits.gapReason),
    schoolTierInsight: cleanSimpleText(unlockedSchools[0] ? `提分后可新增关注 ${unlockedSchools[0]} 等样本，重点看层次变化。` : "先确认当前冲稳保层次，再看专业方向和最低样本线。", simpleNarrativeLimits.schoolTierInsight),
    nextStep: cleanSimpleText(`补充近三次文化成绩，判断 ${Math.ceil(gap / 0.5)} 分提升空间。`, simpleNarrativeLimits.nextStep)
  };
}
...
function normalizeSimpleNarratives(value: unknown, fallback: Record<string, string>) {
  ...
  return simpleNarrativeFields.reduce((result, field) => {
    const maxLength = simpleNarrativeLimits[field] || 48;
